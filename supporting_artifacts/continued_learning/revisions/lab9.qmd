...
***For the regression I was looking for** one **observation per year (e.g., 2001). Your data has one observation per Sex, State, and Year (e.g., F, Colorado, 2001).***
...
***Your geom_bar() plot is plotting one row per Name which ignores the "Count" column which contains the number of Allans with that spelling each Year. So, geom_col() is what you want, so long as you provide the height of the columns (using the Count variable)!***

[I, at first, used geom_col to plot the Allen/Alan/Allan observations. However, I should have summarised by data before plotting to get the correct heights from Count. After summarising, I am now able to make a simple line plot that displays this information much better than before!]{style="color: brown;"}
