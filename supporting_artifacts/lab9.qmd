...
$\widehat{Allisons} = 3881.8 -1.88year$
...
The residuals are right skewed and centered around 0. Because the residuals are the actual - predicted value, we would hope to see residuals centered around 0, as that would indicate the regression doesn't consistently overestimate or underestimate values.

#### 8.

The coefficient on year is negative, meaning that each year, the number of Allisons, or the popularity of the name "Allison," is decreasing, though it is decreasing only slightly, with each year predicted to have 1.88 fewer new Allisons.
...
ggplot(data = names_Allen, aes(x = Year, y = Count)) +
