...
```{r data}
surveys <- read_csv(here::here("supporting_artifacts", "supporting_data", "surveys.csv"), show_col_types = TRUE)
dim(surveys)
...
The surveys data has `r nrow(surveys)` rows and `r ncol(surveys)` columns.
...
The species_id, sex, date_of_week, plot_type, genus, species, and taxa variables are all characters. The record_id, month, day, year, plot_id, hindfoot_length, and weight are doubles. The date variable is a date.
...
ggplot(data = surveys) + 
  geom_jitter(mapping = aes(x = weight, y = hindfoot_length),
              alpha = 0.1) +
  labs(x = "Weight", y = "Hindfoot Length")
...
ggplot(data = surveys,
  ...
  annotate(geom = "label",
           y = 1:14,
           x = 230, 
           label = paste0(c("Neotoma", "Chaetodipus", "Peromyscus",
                            "Perognathus", "Reithrodontomys", "Sigmodon"
                            , "Onychomys", "Peromyscus","Reithrodontomys",
                            "Dipodomys", "Dipodomys", "Chaetodipus",
...
*I used https://stackoverflow.com/questions/39602828/changing-background-color-for-a-text-annotation-to-increase-contrast-and-visibil to add a background color to the annotation*
